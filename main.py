...
"""
 Calculate the AR given the recall and IoU array
 
"""

def vod_ar(rec, IoUlist):
    ar = 0.0
    for i in range(len(IoUlist)):
        #ar += float(max(IoUlist[i] - 0.5, 0.0) / len(IoUlist))
        ar += float(max(IoUlist[i] - 0.5, 0.0) * rec[i]) / len(IoUlist)
    return ar
...
with open(output_files_path + "/output.txt", 'w') as output_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        ar = vod_ar(rec[:], listIoU[:])
        sum_AP += ap
        sum_AR += ar
        text = "{0:.2f}%".format(ap * 100) + " = " + class_name + " AP "  # class_name + " AP = {0:.2f}%".format(ap*100)
        text1 = "{0:.2f}%".format(ar * 100) + " = " + class_name + " AR "  # class_name + " AR = {0:.2f}%".format(ar*100)
        """
         Write to output.txt
        """
        rounded_prec = ['%.2f' % elem for elem in prec]
        rounded_rec = ['%.2f' % elem for elem in rec]
        rounded_iou = ['%.2f' % elem for elem in listIoU]
    ...
    mAR = sum_AR / n_classes
    text = "mAR = {0:.2f}%".format(mAR * 100)
    output_file.write(text + "\n")
    print(text)
